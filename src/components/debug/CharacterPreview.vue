...
import { usePlayerStore } from '@/stores/character' // [!code new]
import { createNewCharacter } from '@/utils/characterFactory'
import { generateItem } from '@/utils/equipmentGenerator'
import { getItemDescription } from '@/models/Item' // [!code new]
...
// --- STATE ---
const player = usePlayerStore() // [!code new]
...
// Helper to render the tooltip using the Logic Library, not the Item class
const tooltipLines = computed(() =>
  tooltipItem.value ? getItemDescription(tooltipItem.value) : [],
)
...
function generatePreview() {
  try {
    // [!code new] Hydrate the store directly with the POJO
    const newState = createNewCharacter('Debug_Preview', previewId.value, archetypesData)
    player.$patch(newState)
  } catch (e) {
    console.error(e)
  }
}
...
function fabricateItem() {
  // Logic mostly remains, but we operate on POJOs now
  ...
  const contentIndex = selectedPrefixData.value.contents.findIndex(
    (c) => c.slot === targetSlot && c.type === targetType,
  )
  if (contentIndex === -1) return

  let targetRarity = gen.rarity
  if (targetRarity === 'random')
    targetRarity = RARITIES[Math.floor(Math.random() * RARITIES.length)]

  let count = gen.statCount === 'random' ? Math.floor(Math.random() * 5) : parseInt(gen.statCount)

  // Generate POJO
  const newItem = generateItem(gen.prefixId, contentIndex, targetRarity, count)

  // [!code fix] Apply forced stats manually to the POJO
  if (gen.statCount !== 'random' && count > 0) {
    const activeForced = gen.forcedStats.slice(0, count)
    if (activeForced.some((s) => s !== null)) {
      newItem.baseStats = {} // Reset randoms
      const rules = equipmentData

      activeForced.forEach((statKey) => {
        if (!statKey) {
          const used = Object.keys(newItem.baseStats)
        ...
        if (statKey) {
          const val = newItem.netValue * rules.stat_conversion[statKey]
          newItem.baseStats[statKey] = (newItem.baseStats[statKey] || 0) + val
        }
      })
    }
  }

  // [!code new] Store Actions
  player.addItem(newItem)
  player.equipItem(newItem.uid)
}

function stripGear() {
  Object.keys(player.equipment).forEach((s) => player.unequipSlot(s))
}

function toggleSlot(slot, isChecked) {
  if (!isChecked) player.unequipSlot(slot)
  else {
    const item = player.inventory.find((i) => i.slot === slot)
    if (item) player.equipItem(item.uid)
  }
}
  ...
  <div class="preview-layout" @mousemove="updateTooltip">
    <div class="sheet-column">
      ...
      <div class="character-sheet">
        <div class="panel">
          <h3>Attributes (Lv. {{ player.level }})</h3>
          <div v-for="(val, stat) in player.stats.attributes" :key="stat" class="stat-row">
            <span>{{ stat.toUpperCase() }}</span>
            <span class="val"
              >{{ val }}
              <span v-if="player.stats.bonuses[stat]" class="bonus-text"
                >(+{{ player.stats.bonuses[stat] }})</span
              ></span
            >
          </div>
          <hr />
          <h3>Combat</h3>
          <div v-for="(val, stat) in player.stats.combatStats" :key="stat" class="stat-row">
        ...
        <div class="panel equipment-grid">
          <div v-for="slot in ARMOR_SLOTS" :key="slot" class="slot-wrapper">
            <div class="slot-toggle">
              <input
                type="checkbox"
                :checked="!!player.equipment[slot]"
                @change="(e) => toggleSlot(slot, e.target.checked)"
                :disabled="
                  !player.equipment[slot] && !player.inventory.find((i) => i.slot === slot)
                "
              />
            </div>
            <div
              class="slot-box"
              :class="{ filled: player.equipment[slot] }"
              @mouseenter="showTooltip(player.equipment[slot], $event)"
              @mouseleave="hideTooltip"
            >
              <span class="slot-label">{{ slot }}</span>
              <span v-if="player.equipment[slot]" class="item-name">{{
    ...
    <div class="generator-column panel">
      ...
      <div class="stats-selectors">
        ...
        <div v-for="n in 4" :key="n" class="stat-select-row">
          <select
            v-model="gen.forcedStats[n - 1]"
            :disabled="gen.statCount === 'random' || n > parseInt(gen.statCount)"
          >
    ...
    >
      <div v-for="(line, i) in tooltipLines" :key="i" :class="{ 'tt-head': i === 0 }">
        {{ line }}
      </div>
    </div>
  </div>
...
.stat-select-row select:disabled {
  opacity: 0.3;
  cursor: not-allowed;
}
.fabricate-btn {
